...
#%% --- Visualization Five: Choropleth map + distribution ( diff color )

with plt.style.context('matplotlib_stylesheet_ejg_fixes'):
    
    # --- Create figure and axes ---
    fig_5 = plt.figure(figsize = (19.20,19.20))
    
    gs = fig_5.add_gridspec(2,8,
                            figure = fig_5,
                            top = 0.50, bottom = 0,
                            wspace = 0, hspace = 0)
    
    ax_1 = fig_5.add_subplot(gs[0,:-1])
    ax_2 = fig_5.add_subplot(gs[1,:-1])
    ax_3 = fig_5.add_subplot(gs[:,-1:])
    ...
    # --- set facecolor ---
    #ax_3.set_facecolor("#D5E8EB")
    ax_3.set_facecolor("#D2D8D9")
...
filenames = ["choropleth", "actual", "two_maps", "maps_barchart_g1", "maps_barchart_g2"]
...
for figure, filename in zip(figures, filenames):
    for file_extension in file_extensions:
        ...
        figure.savefig(export_fp,
                        ...
                        bbox_inches = "tight",
                        facecolor = (lambda x: "#D0CFD4" if figure == fig_4 else "#D2D8D9")(figure),
                        pad_inches = 0)